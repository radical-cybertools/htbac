...
class Esmacs_MPI(object):
    ...
    def generate_pipeline(self):
        ...
        for replica in range(self._replicas):
            ...
            task_path = s1_ref["replica_{0}_step_{1}".format(replica, self.workflow[count-1])+'/'+self.rootdir]
        ...
        for replica in range(self._replicas):
            ...
            #we obtain the task path of the previous step for current replica
            task_path = s2_ref["replica_{0}_step_{1}".format(replica, self.workflow[count-1])+'/'+self.rootdir]
        ...
        for replica in range(self._replicas):
            t = Task()
            t.name = "replica_{0}_step_{1}".format(replica,self.workflow[count])
            task_path = s3_ref["replica_{0}_step_{1}".format(replica, self.workflow[count-1])+'/'+self.rootdir]
