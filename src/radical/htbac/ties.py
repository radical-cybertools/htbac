...
class Ties(object):
    ...
    def __init__(self, number_of_replicas, number_of_windows, system, cores=32,
                workflow=None, ligand=False, full=False, additional=list()):
    ...
    def generate_pipeline(self):
        ...
        for step in self.workflow:
            ...
            box = pmd.amber.AmberAsciiRestart('systems/ties{lig}/{s}/build/{s}-complex.crd'.format(lig=self.ligand, s=self.system)).box
                
            for replica in range(self.number_of_replicas):
                for ld in self.lambdas:
                
                    task = Task()
                    task.name = 'system_{}_replica_{}_lambda_{}'.format(self.system, replica, ld)
    ...
    @property
    def replicas(self):
        return self.number_of_replicas*len(self.lambdas)*len(self.system)
     @property
    def total_cores(self):
        return self.number_of_replicas*len(self.lambdas)*self.cores
