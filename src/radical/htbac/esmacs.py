...
class Esmacs(object):
    ...
    def generate_pipeline(self):
        ...
        for replica in range(self.replicas):
            ...
            t.copy_input_data.append(task_path+'/replicas/rep{input1}/equilibration/{input2}.coor > {input3}/replicas/rep{input1}/{input4}/{input2}.coor'.format(input1 = replica, 
                                                 input2 = self.workflow[count-1], 
                                                 input3 = self.rootdir,
                                                 input4 = self.workflow[count]))

            t.copy_input_data.append(task_path+'/replicas/rep{input1}/equilibration/{input2}.vel > {input3}/replicas/rep{input1}/{input4}/{input2}.vel'.format(input1 = replica,
                             input2 = self.workflow[count-1],
                             input3 = self.rootdir,
                             input4 = self.workflow[count]))
        ...
        for replica in range(self.replicas):
            ...
            t.cpu_reqs = self.cpu_reqs
            t.pre_exec = ['export OMP_NUM_THREADS=1', 
                          "sed -i 's/REPX/{input2}/g' {input1}/sim_confs/*.conf".format(input1 = self.rootdir, 
                                                                                input2 = replica)]
