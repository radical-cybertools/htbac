...
# define step 4: 
s3 = Simulation(name='production_0')
s3.engine = 'namd'
s3.processes = 32
s3.threads_per_process = 1
s3.add_ensemble('replica', range(5))
s3.add_ensemble('lambdawindow', [1.00, 0.50, 0.00])
...
# compute new lambda windows using AdaptiveQuadrature

aq = AdaptiveQuadrature(output_name = "dg.out")

# end of first protocol

p1 = Protocol()

s3 = Simulation(name='production_1')
s3.engine = 'namd'
s3.processes = 32
s3.threads_per_process = 1
s3.add_ensemble('replica', range(5))
s3.add_ensemble('lambdawindow', [1.00, 0.50, 0.00, aq.requested_windows()])
s3.add_input_file("default_configs/rfe/54353507-54150798/sim.conf", is_executable_argument=True)
s3.system = system
...
ht.rerun(self, protocol=p1, terminate=True, previous_pipeline=p0)
